...
async function main() {
  ...
  await step('CREATE sale agent under store', async () => {
    ...
    expect(
      read?.store_id === storeId && read.ratio_pct?.toString() === '12.5',
      'hierarchy + ratio persisted'
    );
  });
  ...
  await step('CREATE / UPDATE / UNIQUE(store,platform)', async () => {
    ...
    try {
      await db.store_platform_accounts.create({
        data: { store_id: storeId, platform_id: platformId },
      });
    } catch {
      threw = true;
    }
  ...
  await step('CREATE / UPDATE / UNIQUE(store,username)', async () => {
    ...
    try {
      await db.users.create({
        data: { username: U('member'), nickname: 'dup', invite_code: 'dup', password_hash: hash },
      });
    } catch {
      threw = true;
    }
