...
import { api, Btn, Field, Modal, Select, Table, TextInput, Toggle } from '../ui';
...
export function PlatformCatalogScreen() {
  ...
  const remove = async (p: Platform) => {
    if (!window.confirm(`Deactivate "${p.name}"? It will be hidden from stores but its history is kept.`))
      return;
    await api(`/api/agent/platforms?id=${encodeURIComponent(p.id)}`, { method: 'DELETE' });
    void load();
  };
